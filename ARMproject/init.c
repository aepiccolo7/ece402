...
void init_ADC(void)
{
    SysCtlPeripheralEnable(SYSCTL_PERIPH_GPIOD); //control lines
    ...
    GPIOPinTypeGPIOOutput(GPIO_PORTD_BASE, GPIO_PIN_1|GPIO_PIN2);	//2 control lines
...
void init_DAC(void)
{
	...
	GPIOPinTypeGPIOOutput(GPIO_PORTD_BASE, GPIO_PIN_0); //1 control line
}
